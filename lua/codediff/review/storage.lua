...
local function data_dir()
  local configured = config.get().storage_dir
  if configured and configured ~= "" then
    return configured
  end
  return vim.fn.stdpath("data") .. "/codediff-review"
end
...
local function get_git_branch()
  local ok, result = pcall(function()
    return vim.system({ "git", "rev-parse", "--abbrev-ref", "HEAD" }, { text = true }):wait()
  end)
  if not ok or not result or result.code ~= 0 then
    return nil
  end
...
function M.get_storage_path()
  ...
  if current_revisions then
    ...
    return string.format("%s/%s-%s_%s.json", dir, project_hash, r1, r2)
  end
  ...
  return string.format("%s/%s-%s.json", dir, project_hash, safe_branch)
end
...
function M.save(comments)
  ...
  local encoded = vim.fn.json_encode(comments)
