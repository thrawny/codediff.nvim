  ...
  for _, line in ipairs(result) do
    ...
    if #parts >= 5 then
      table.insert(parsed, {
        ...
        author = parts[3],
        date = parts[4],
        message = table.concat({ unpack(parts, 5) }, "|"),
      })
    end
  end
...
local function render_lines()
  ...
  ns_id = vim.api.nvim_create_namespace("codediff_review_picker")
...
function M.open(callback)
  local git_root = get_git_root()
  if not git_root then
    vim.notify("Not in a git repository", vim.log.levels.ERROR, { title = "codediff.review" })
    return
  end
  ...
  range_end = nil
  if #commits == 0 then
    vim.notify("No commits found", vim.log.levels.WARN, { title = "codediff.review" })
    return
  end
