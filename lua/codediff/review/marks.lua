...
local ns_id = vim.api.nvim_create_namespace("codediff_review")
local ns_padding = vim.api.nvim_create_namespace("codediff_review_padding")
...
function M.align_buffers(orig_buf, mod_buf, orig_file, mod_file)
  ...
  if not orig_buf or not mod_buf or not vim.api.nvim_buf_is_valid(orig_buf) or not vim.api.nvim_buf_is_valid(mod_buf) or (not orig_file and not mod_file) then
    return
  end
...
function M.refresh()
  local ok, hooks = pcall(require, "codediff.review.hooks")
  if not ok then
    return
  end
