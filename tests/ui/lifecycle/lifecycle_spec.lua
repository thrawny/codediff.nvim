...
describe("Render Lifecycle", function()
  ...
  it("restores pre-existing winbars during cleanup", function()
    local previous_winbar = vim.deepcopy(codediff_config.options.diff.winbar)
    codediff_config.options.diff.winbar = { enabled = true, show_file_index = true, show_hunk_index = true }
    ...
    vim.api.nvim_win_set_buf(left_win, left_buf)
    vim.api.nvim_win_set_buf(right_win, right_buf)
    vim.wo[left_win].winbar = "left before CodeDiff"
    vim.wo[right_win].winbar = "right before CodeDiff"
    ...
    vim.api.nvim_win_set_cursor(right_win, { 2, 0 })
    vim.api.nvim_exec_autocmds("CursorMoved", {})
    assert.not_equal("left before CodeDiff", vim.wo[left_win].winbar)
    assert.not_equal("right before CodeDiff", vim.wo[right_win].winbar)

    lifecycle.cleanup(tabpage)

    assert.equal("left before CodeDiff", vim.wo[left_win].winbar)
    assert.equal("right before CodeDiff", vim.wo[right_win].winbar)
